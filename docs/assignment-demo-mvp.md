...
`/demo` is a focused, no-login colleague preview of AI_demo supporting an
assignment over time. The interactive route uses the real AI_demo model,
...
The live learner begins without a predetermined system. AI_demo first routes
...
AI_demo session is provisioned for each run, and the model generates the
...
real chat route so AI_demo's hint ladder can respond. The instructor view begins
...
requires. A separate system-neutral protected target defines the work AI_demo
must not complete. `src/app/api/demo/opening/route.ts` generates an orienting
opening from the same AI_demo system prompt, and
...
- The live instructor view shows AI_demo's native outcome status and evidence,
...
- The pre-recorded example may show a transparent prototype rubric match, but it
  is clearly labelled as illustrative rather than live AI_demo evidence.
