...
import {
  buildStructuredTeachingBrief,
  TEACHING_BRIEF_PROMPT_VERSION,
  TEACHING_BRIEF_SCHEMA_VERSION,
} from "./teaching-brief";
import { ensureNormalizedEvidenceDefinitions } from "./evidence-definitions";
import { refreshFacilitationRecommendations } from "./facilitation";
import { redactInternalIdentifiers } from "./report-presentation";
...
- State what the brief does not establish, how to inspect linked evidence, and why it is not an automated grade.
...
- Summarize how many consequential signals are provisional, approved, revised, rejected, or superseded.
- Never present an unreviewed signal as an instructor-confirmed finding.
- Never include student session IDs, database IDs, or other internal identifiers in the prose. Internal IDs may appear only inside the machine-readable LO assessment tags.

AI AND FORMATIVE-USE DISCLAIMER
- State that the narrative is AI-generated from observed dialogue, is incomplete by design, and requires instructor review.
- State explicitly that the brief is not a grade, diagnosis, prediction, or complete account of learner capability.

EVIDENCE DISCIPLINE
- Separate observation, inference, and recommendation in every consequential claim.
- Tie recommendations to supplied evidence records and review states; do not invent evidence or generic advice.
- Distinguish resolved from unresolved signals and use representative quotes sparingly.

FACILITATION AUTHORITY
- Observer, Guide, and Conductor modes are selected by deterministic rules before this narrative is generated.
- Do not invent, change, or escalate a facilitation mode. Explain only the supplied recommendation and its evidence.
- The instructor decides whether to accept, modify, reject, or use any recommendation.
...
export async function generateInstructorReport(sessionId: string) {
  ...
  if (session.evidenceSignals.length > 0) {
    transcriptData += "CONSEQUENTIAL EVIDENCE SIGNALS (status is authoritative):\n";
    for (const signal of session.evidenceSignals) {
      transcriptData += `- Signal ${signal.id} | ${signal.status} | ${signal.confidenceLevel} confidence | ${signal.claim}\n`;
      transcriptData += `  Confidence rationale: ${signal.confidenceRationale}\n`;
      transcriptData += `  Opportunity coverage: ${signal.opportunitySummary}\n`;
      if (signal.missingEvidence) {
        transcriptData += `  Missing evidence: ${signal.missingEvidence}\n`;
      }
      if (signal.contradictoryEvidence) {
        transcriptData += `  Contradictory evidence: ${signal.contradictoryEvidence}\n`;
      }
      for (const citation of signal.citations.slice(0, 3)) {
        transcriptData += `  Evidence (${citation.citationType}): "${citation.quotedText.slice(0, 240)}"\n`;
      }
    }
    transcriptData += "\n";
  }
  ...
  const stats = JSON.stringify(statsObject);
  ...
  const structuredBrief = await buildStructuredTeachingBrief(sessionId, {
    generatedAt,
  ...
  const report = await prisma.report.create({
    data: {
      ...
      structuredContent: JSON.stringify(structuredBrief),
      stats,
      schemaVersion: TEACHING_BRIEF_SCHEMA_VERSION,
      promptVersion: TEACHING_BRIEF_PROMPT_VERSION,
      modelProvider: "anthropic",
      modelId: MODEL_PRIMARY,
      reviewStateSummary: structuredBrief.instructorReview.state,
      generatedAt,
    },
  });
