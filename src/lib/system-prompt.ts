...
export function buildSystemPrompt(
  ...
  if (session?.planningOpeningQuestion) {
    prompt += `\n\nINSTRUCTOR-PLANNED OPENING QUESTION\nUse this exact prior-knowledge question in the opening exchange: ${session.planningOpeningQuestion}`;
  }

  if (session?.planningTaskInstructions) {
    prompt += `\n\nINSTRUCTOR-PLANNED TASK\n${session.planningTaskInstructions}`;
  }

  if (session?.planningIntendedOutput) {
    prompt += `\n\nINTENDED USABLE OUTPUT\nHelp the learner produce this by the end of the session without writing it for them: ${session.planningIntendedOutput}`;
  }
