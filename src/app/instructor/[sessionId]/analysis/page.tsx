...
export default function SessionAnalysisPage() {
  ...
  const fetchReport = useCallback(
    async (forceRefresh = false) => {
      ...
      try {
        ...
        let data = (await res.json()) as ReportData & {
          error?: string;
          code?: string;
          regenerationAllowed?: boolean;
        };
        if (
          ...
          res = await fetch(`/api/sessions/${sessionId}/report`, {
            method: "POST",
          });
          data = await res.json();
        }
        if (!res.ok) throw new Error(data.error || "Failed to generate or fetch report");
        setReport(data);
      } catch (err) {
  ...
  const handleExport = () => {
    if (report?.stale) {
      setToast({
        tone: "error",
        message: "Refresh the teaching brief before exporting the PDF.",
      });
      return;
    }
  ...
  return (
    <main className="minerva-page">
      <div className="minerva-shell space-y-6 py-8">
        ...
        <div className="minerva-card p-6 md:p-8">
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {effectiveView === "brief" && report?.regenerationAllowed !== false && (
                <button
        ...
        {report?.stale && (effectiveView === "brief" || effectiveView === "quick") ? (
          <div
            className="rounded-xl border border-[rgba(144,111,18,0.28)] bg-[rgba(144,111,18,0.08)] px-6 py-4 text-sm text-[var(--charcoal)]"
            role="status"
          >
            <p className="font-semibold">This teaching brief needs to be refreshed.</p>
            <p className="mt-1 text-[var(--dim-grey)]">
              Evidence or recommendations changed after it was generated. Click Refresh brief before exporting.
            </p>
          </div>
        ) : null}
          ...
          <>
            ...
            {report?.brief ? (
              <TeachingBriefFacilitationPivots
                sessionId={sessionId}
                pivots={report.brief.facilitationPivots}
              />
            ) : null}
                ...
                <p className="text-sm text-[var(--dim-grey)]">
                  {loadingRecommendations
                    ...
                    : recommendationMessage ||
                      "Recommendations will appear after learners have produced enough evidence. You can refresh after learner activity begins."}
                </p>
              </div>
            ) : (
