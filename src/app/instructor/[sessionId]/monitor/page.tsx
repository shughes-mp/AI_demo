...
export default function StudentMonitorPage() {
  ...
  if (isLoading) {
    return <LoadingState variant="page" message="Loading learner activity…" />;
  }
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            <div>
              <nav className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--dim-grey)]">
                ...
                <span>/</span>
                <span className="text-[var(--charcoal)]">{INSTRUCTOR_LABELS.monitor}</span>
              </nav>
              <h1 className="mt-4 font-serif text-[42px] leading-[0.96] tracking-[-0.03em] text-[var(--charcoal)]">
                {INSTRUCTOR_LABELS.monitor}
              </h1>
        ...
        {students.length === 0 ? (
          <div className="minerva-card p-8">
            ...
            <p className="mt-3 max-w-[34rem] text-sm text-[var(--dim-grey)]">
              When learners join with the session link, their activity, possible follow-up needs, and conversations will appear here.
            </p>
          </div>
        ) : (
          <>
                                  ...
                                  <div className="mt-2 space-y-3 border-l-2 border-[var(--rule)] pl-3">
                                    {student.reviewSignals.map((signal: any) => (
                                      <div key={`${signal.type}-${signal.relevantExchange}`}>
                                        ...
                                        <p className="mt-1">Why: {signal.whyFlagged}</p>
                                        <p className="mt-1">Uncertainty: {signal.uncertainty}</p>
                                        <p className="mt-1">Suggested action: {signal.suggestedAction}</p>
                                        <time
                                          dateTime={new Date(signal.occurredAt).toISOString()}
