        ...
        <section className="section-rule grid grid-cols-1 md:grid-cols-[156px_360px_minmax(0,1fr)]">
          ...
          <div className="px-4 py-14 md:px-8 md:py-16">
            ...
            {session.planningTaskInstructions && (
              <div className="mt-7 max-w-[28rem] border-l-2 border-[var(--teal)] pl-4">
                <p className="text-[11px] font-semibold uppercase tracking-[0.1em] text-[var(--teal)]">Your task</p>
                <p className="mt-2 text-[14px] leading-6 text-[var(--charcoal)]">{session.planningTaskInstructions}</p>
                {session.planningIntendedOutput ? (
                  <p className="mt-2 text-[13px] leading-6 text-[var(--dim-grey)]"><strong>Finish with:</strong> {session.planningIntendedOutput}</p>
                ) : null}
              </div>
            )}
